...
public class DeltaLog {
    ...
    /**
     * Reconstructs the table state as of a specific version, by replaying commits 0..version.
     *
     * <p>Mirrors {@code DeltaLog.getSnapshotAt(version)}. This is what makes time travel
     * almost free: reading an old version costs a metadata replay, not a reprocessing of
     * data. Commits are immutable, so version N always reconstructs identically.
     *
     * <p>Real Delta replays from the nearest checkpoint rather than from 0. We always start
     * at 0 — correct, and fine at workshop scale; see MAPPING.md.
     *
     * @param version the version to reconstruct, or -1 for an empty table
     * @throws IOException if an I/O error occurs
     */
    public Snapshot getSnapshotAt(long version) throws IOException {
        List<Action> actions = new ArrayList<>();
        for (Long v : listVersions()) {
            if (v > version) {
                break;   // versions are sorted; anything later is not part of this snapshot
            }
            actions.addAll(readVersion(v));
        }
        return new Snapshot(this, version, actions);
    }
