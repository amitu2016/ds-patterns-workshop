...
func (rsc *ReplicaSetController) Reconcile(ctx context.Context, rs *api.ReplicaSet) error {
	...
	if currentPodCount < desiredPodCount {
		...
		// Scale down. The reconciler owns both directions of the diff — a loop that
		// only ever creates is not reconciling, it is reacting.
		//
		// Real Kubernetes ranks victims before deleting (getPodsToDelete: unassigned
		// before assigned, Pending before Running, newest before oldest) so that
		// scaling down costs the least. We delete from the end of the owned list,
		// which is deterministic and enough to show the loop converging.
		excess := currentPodCount - desiredPodCount
		for i := 0; i < excess; i++ {
			victim := activePods[currentPodCount-1-i]
			if err := rsc.podRegistry.DeletePod(ctx, victim.Name); err != nil {
				return fmt.Errorf("failed to delete pod %s while scaling down %s: %v",
					victim.Name, currentRS.Name, err)
			}
		}

		currentPodCount = desiredPodCount
		// Status is written only after the deletions actually landed, so it never
		// claims a convergence that did not happen.
		currentRS.Status.Replicas = int32(currentPodCount)
		return rsc.replicaSetRegistry.Update(ctx, currentRS)
	}
