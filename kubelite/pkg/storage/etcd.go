...
func StartEmbeddedEtcdWithPort(peerPort, clientPort int) (*embed.Etcd, int, error) {
	...
	// Demos must stay readable on a screen share. Embedded etcd logs its whole raft
	// startup at INFO in JSON, which buries a demo's narrative under ~30 lines before
	// anything interesting happens. Errors still surface.
	//
	// This is the Go counterpart of config/logging/logback-test.xml on the Java side.
	// Raise to "info" temporarily when etcd's own behaviour is what you want to show.
	cfg.Logger = "zap"
	cfg.LogLevel = "error"
...
func createTempDir() (string, error) {
	...
	tempDir, err := os.MkdirTemp("", "kubelite-test-")
	if err != nil {
