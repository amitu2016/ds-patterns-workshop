...
func NewKubelet(nodeName, apiServerURL string) (*Kubelet, error) {
	...
	return NewKubeletWithClient(nodeName, apiServerURL, dockerClient), nil
}

func NewKubeletWithClient(nodeName, apiServerURL string, dockerClient *client.Client) *Kubelet {
	return &Kubelet{
		nodeName:     nodeName,
...
func (k *Kubelet) StartContainer(ctx context.Context, pod *api.Pod, containerName, imageName string) error {
	...
	labels := map[string]string{
		"kubelite.pod.name":       pod.Name,
		"kubelite.pod.namespace":  pod.Namespace,
		"kubelite.container.name": containerName,
	}
...
func (k *Kubelet) ListContainers(ctx context.Context) ([]ContainerStatus, error) {
	...
	for _, c := range containers {
		podName, ok := c.Labels["kubelite.pod.name"]
		if !ok {
		...
		for _, containerSpec := range pod.Spec.Containers {
			if containerSpec.Name == c.Labels["kubelite.container.name"] {
...
func (k *Kubelet) CleanupContainers(ctx context.Context) error {
	...
	for _, c := range containers {
		if podName, ok := c.Labels["kubelite.pod.name"]; ok {
